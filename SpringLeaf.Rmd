...
for (i in 1:ncol(chr.cols)) {
  for (j in 1:nrow(chr.cols[i])) {
    if(is.na(chr.cols[j,i])) {
       chr.cols[j,i] <- -1 
    } 
    }
}  
...
#test for NAs 
ifelse(map_int(chr.cols, function(x) {sum(is.na(x))}) == 0, "None", "Try Again")
...
chr.cols.omit.NA <- train  %>% select(ID, chr.cols) %>% filter_all(any_vars(is.na(.))) #%>% select(ID)
      ### ~ identical to --> train %>% filter_if(is.character, any_vars(is.na(.))) %>% select_if(is.character) 
...
map_int(chr.cols, function(x) {sum(is.na(x))}) == 0, "No NAs", 0)

# if the count of NAs is non-zero, apppend the column names to a new vector
ifelse(map_int(wtf, ~function(x) {(is.na(x))}) != TRUE, 1, 0) %>% ifelse(.>0,1,0)

wtf %>% detect_index(is.na)


train[ifelse(map_chr(train, type_sum) == "chr", TRUE,FALSE)] %>%  filter_all(any_vars(is.na(.)))
